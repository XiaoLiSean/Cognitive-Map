...
from Network.retrieval_network.params import IMAGE_SIZE, DATA_DIR, TRIPLET_FILE_NAME, PAIR_FILE_NAME
...
class PairDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir=DATA_DIR, pair_file_name=PAIR_FILE_NAME, image_size=IMAGE_SIZE, is_train=False, is_val=False, is_test=False, load_only_image_data=False):
    ...
    def load_pairs(self):
        ...
        # Iterate through floorplans
        for FloorPlan in os.listdir(path):
            incoming_pairs = np.load(path + '/' + FloorPlan + '/' + self.pair_file_name) # load npy list of pairs
            for pair in incoming_pairs:
                A = path + '/' + FloorPlan + '/' + pair[0]
                B = path + '/' + FloorPlan + '/' + pair[1]
                pairs_data.append(deepcopy((A, B)))
    ...
    def __getitem__(self, index):
        ...
        if self.load_only_image_data:
            ...
            return pair_imgs + adj_on + adj_in + adj_proximity + fractional_bboxs + obj_occurence_vecs
