...
class Robot():
	def __init__(self, scene_type, scene_num, save_directory, overwrite_data=False, AI2THOR=False, grid_size=0.25, rotation_step=90, sleep_time=0.005, use_test_scene=False, debug=False):
		...
		self._localization_network = None
	...
	def Set_localization_network(self, network=None):
		...
		self._localization_network = SiameseNetImage()
		self._device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
		print('setting localization network')
		self._localization_network.to(self._device)
		self._localization_network.load_state_dict(torch.load(checkpoint))
		self._localization_network.eval()
		return
	...
	def Navigation_stop(self, image_goal, image_current, goal_pose=None, hardcode=False):
		...
		localized = is_localized_static(self._localization_network, self._device, Image_goal, Image_current)
